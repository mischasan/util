...
Railgun7w(char *pbTarget, int cbTarget, char *pbPattern, int cbPattern)
{
    return (char*)(
         ...
         : cbPattern < 3        ? NULL  //XXX dummy something up, even just "do cp = strchr(cp, pat[1]); while (cp && cp[-1] != pat[0]);"
...
//XXX make this do a real BMH skipahead by computing filter[...] = cbPattern - i - 2;
...
bloom2(byte *pbTarget, byte *pbPattern, uint cbTarget, uint cbPattern)
{
    ...
    for (i = 0; i <= cbTarget - cbPattern; i += skip) {
        if (!filter[*(uint16_t*) &pbTarget[i + cbPattern - 2]])    // Test YZ
            skip = cbPattern - 1;
        else if (2 > filter[*(uint16_t*) &pbTarget[i + cbPattern - 4]] // X+Y<2 vs !(X|Y) 
