...
*/
#include "_psearch.h"
// psearch_strings: reconstruct the input to psearch_create.
// Returns: number of strings in strv[].
// strv[] entries are returned in lexical order, not
// necessarily the original order given to psearch_create.
...
static void do(VISIT*, PSEARCH const *psp, TRAN base, int depth);

MEMREF*
psearch_strings(PSEARCH const *psp, int *pnstrs)
{
    int seqv[psp->nsyms];
...
static void
do(VISIT *vp, PSEARCH const *psp, TRAN base, int depth)
{
