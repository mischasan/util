...
static void fill_tranv(PSEARCH*, TNODE const*);
...
static inline void
set_tran(PSEARCH *psp, STATE s, SYMBOL sym, int match, int suffix, TRAN ns)
{
...
// Track statistics during construction.
#ifdef STAT
...
//--------------|---------------------------------------------
PSEARCH*
psearch_create(MEMREF const* strv, int nstrs)
{
    TNODE *tp, **v1 = NULL, **v2 = NULL;
    PSEARCH *psp = calloc(1, sizeof*psp);
    ...
    goto DONE;
FAIL: psearch_destroy(psp), psp = NULL;
...
static void
fill_symv(PSEARCH *psp, MEMREF const *strv, int nstrs)
{
    int i, j;
    FRANK frv[256];
...
static void
fill_hashv(PSEARCH *psp, TNODE const treev[], int nnodes)
{
...
static void
fill_tranv(PSEARCH *psp, TNODE const*tp)
{
