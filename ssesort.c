...
//  Order-preserving over duplicates.
//NOTE: This unrolling and byte-switching leaves ssesrank16d speed
//  only 3 x qsort and 2 x insort. ssesort16d is 10 x qsort.

#define LSB(x) (*(uint8_t*)&(x))
#define DO_0_14 _F(0) _F(1) _F(2) _F(3) _F(4) _F(5) _F(6) _F(7) \
                _F(8) _F(9) _F(10) _F(11) _F(12) _F(13) _F(14) 
#define DO_0_15 DO_0_14 _F(15) 
void
...
#   define _F(i) dist[15 & LSB(keys[i])]++;
    DO_0_15;

#   undef _F
#   define _F(i) dist[(i)+1] += dist[i];
    DO_0_14; // NOT DO_0_15.

#   undef _F
#   define _F(i) LSB(keys[i]) = (lsb[i] = LSB(keys[i]) & 0xF0) | --dist[LSB(keys[i])];
    DO_0_15;

    ssesort16d(keys);

#   undef _F
#   define _F(i) LSB(keys[i]) = lsb[rank[i] = LSB(keys[i]) & 15];
    DO_0_15;
}
