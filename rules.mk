...
#NOTE: defining SIZEOF_LONG_INT suppresses inclusion of UNIXODBC files in Simba ThirdParty/odbcheaders/*.h
CPPFLAGS        += -DSIZEOF_LONG_INT=8

#NOTE: why PLATFORM_DARWIN? use __APPLE__ !
CPPFLAGS.Darwin += -DPLATFORM_DARWIN
...
Install         = if [ $(words $2) -gt 0 ]; then mkdir -p $1; pax -rwpe -s:.*/:: $2 $1; fi

install         : $(addprefix install., bin etc include ini lib man1 man3 sbin)
install.man%    : $(install.man%)   ; $(call Install,$(DESTDIR)/man/$(substr install.,,$@),$($@))
install.%       : $(install.%)      ; $(call Install,$(DESTDIR)/$(subst install.,,$@),$($@))
