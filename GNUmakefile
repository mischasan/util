...
all		+= util
util.clean 	= $(util)/psearch.tmp $(util)/words $(util)/88.tab
...
$(util)/ordhuff.o $(util)/ordhuff.s : CFLAGS += -fforward-propagate -funroll-all-loops -fvariable-expansion-in-unroller

# source: source files not currently used by any targets.
source += $(util.scripts) $(patsubst %,$(util)/%, .gdbinit aa.c bergstr.c bloom_t.c bndmem.c _concur.h concur.h concur_t.c intsum_t.c psearch.h psearch_strings.c railgun7*.c search_t.c *ssearch.h strstr42.asm strstr.S winstr.c xmm.txt)
