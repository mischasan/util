...
##This messes up "make source" because ordhuff.s is not a dependency of (all test cover profile)
...
#source += $(util.scripts) $(addprefix $(util)/, .gdbinit aa.c bergstr.c bloom_t.c bndmem.c _concur.h concur.h concur_t.c intsum_t.c jlu32.c psearch.h psearch_strings.c railgun7*.c search_t.c *ssearch.h strstr42.asm strstr.S winstr.c xmm.txt)
source += $(addprefix $(util)/, jlu32.c)
