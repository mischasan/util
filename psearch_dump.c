...
static void printrans(PSEARCH const*,TRAN,const char*,FILE*,MEMREF const*);
static void printree(PSEARCH const* psp,
                        int state,
                        int depth,
                        char* str,
                        const char* charv,
...
//--------------|---------------------------------------------
void
psearch_dump(PSEARCH const* psp, PS_DUMP_TYPE pdt, FILE *out, MEMREF const*pattv)
{
    int i, empty;
    char charv[256];
    int symdist[257] = {};
    ...
    if (pdt & PS_STATS) {
        ...
        fprintf(out, "strs:%d syms:%d chars:%d "
                ...
                (long)sizeof(PSEARCH) + p_size(psp));
#if 0
...
static void
printrans(PSEARCH const*psp, STATE s, char const *charv,
...
static void
printree(PSEARCH const*psp, int state, int depth, char *str,
